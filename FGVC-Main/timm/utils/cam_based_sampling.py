...
def get_category_list(dataset, num_classes):
    num_list = [0] * num_classes
    cat_list = []

    for (image_dir ,category_id ) in dataset.parser.samples:
        num_list[category_id] += 1
        cat_list.append(category_id)
    return num_list, cat_list

def cam_based_sampling(dataset, model,args,data_config,CAM_DATA_SAVE_PATH,CAM_DATA_JSON_SAVE_PATH):
    ...
    num_list, cat_list = get_category_list(dataset, num_classes)

    CAM_DATA_SAVE_PATH = CAM_DATA_SAVE_PATH
    CAM_DATA_JSON_SAVE_PATH = CAM_DATA_JSON_SAVE_PATH
    CAM_NUMBER_THRES = 100

    if not os.path.isdir(CAM_DATA_SAVE_PATH):
        os.makedirs(CAM_DATA_SAVE_PATH)
    ...
    print('threshold is: {}'.format(CAM_NUMBER_THRES ))
    print('\n')

    for i in range(len(num_list)):
        if num_list[i]>=CAM_NUMBER_THRES :
            num_absent_list[i] = 0
        elif num_absent_list[i] > CAM_NUMBER_THRES :
            num_absent_list[i] = CAM_NUMBER_THRES
    ...
    for i in range(len(num_absent_list)):
        for j in range(num_absent_list[i]):
            ...
            origin_img_dir = dataset.parser.samples[new_image_index][0]
            origin_img = Image.open(origin_img_dir).convert('RGB')
            #print(origin_img_dir)
            origin_img = np.array(origin_img)
            h, w, _ = origin_img.shape
            #print(origin_img_dir,new_image_index)
            cam_groundtruth = model.get_CAM_with_groundtruth([new_image_index], dataset, label_list, (w, h),args,data_config)
            ...
            fpath = os.path.join(CAM_DATA_SAVE_PATH, 'label_'+str(label_list[new_image_index])+'_generate_index_'+str(generate_index)+'.jpg')
    ...
    import json
    json.dump(cam_generation_data, open(CAM_DATA_JSON_SAVE_PATH , 'w'))

    print('The sampled images have been save to ', CAM_DATA_SAVE_PATH)
    print('The json file of balanced dataset has been saved to ', CAM_DATA_JSON_SAVE_PATH)
