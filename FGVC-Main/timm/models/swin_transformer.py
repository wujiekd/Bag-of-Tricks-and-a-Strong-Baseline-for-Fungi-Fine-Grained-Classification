...
class SwinTransformer(nn.Module):
    ...
    def cam_params_reset(self):
        self.classifier_weights = np.squeeze(list(self.head.parameters())[0].detach().cpu().numpy())

    def get_CAM_with_groundtruth(self, image_idxs, dataset, label_list, size,args,data_config):
        ...
        for i in range(len(image_idxs)):
            ...
            self.eval()
            with torch.no_grad():
                origin_img_dir = dataset.parser.samples[idx][0]
                img = Image.open(origin_img_dir).convert('RGB')
                img = transform(img).unsqueeze(0)
                feature_conv = self.forward_feature_maps(img.to('cuda')).detach().cpu().numpy()
