...
def set_environment(args, tlogger):
    ...
    schedule, num_epochs = create_scheduler(args, optimizer)
...
def train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader):
    ...
    for batch_id, (ids, datas, labels) in enumerate(train_loader):
        ...
        losses_m.update(reduced_loss.item(), datas.size(0))
...
def main(args, tlogger):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if train_loader is not None:
            ...
            lr,train_loss = train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader)
            tlogger.print()
        else:
        ...
        if epoch == 0 or (epoch + 1) % args.eval_freq == 0:
            ...
            if val_loader is not None:
                ...
                acc, eval_name, accs, f1score,val_loss= evaluate(args, model, val_loader)
            ...
            if f1score > best_f1:
                best_f1 = f1score
                # best_eval_name = eval_name
                torch.save(checkpoint, args.save_dir + "backup/best_f1.pt")
            if args.use_wandb:
                wandb.run.summary["best_acc"] = best_acc
                wandb.run.summary["best_eval_name"] = best_eval_name
                wandb.run.summary["best_epoch"] = epoch + 1
        if schedule is not None:
            schedule.step(epoch + 1, float(train_loss))
        writer.add_scalar('train/loss', train_loss, epoch)
        writer.add_scalar('train/lr', lr, epoch)
        writer.add_scalar("valid/loss", val_loss, epoch)
        writer.add_scalar("valid/top1", acc, epoch)
        # writer.add_scalar("valid/top5", eval_metrics['top5'], epoch)
        writer.add_scalar("valid/f1_score", f1score, epoch)
